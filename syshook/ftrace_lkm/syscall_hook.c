...
* Year:		2018
* File:		syscall_hook.c
* Description:	hook execve,connect,accept,accept4,ptrace,init_module,finit_module,open,openat,creat,recvfrom syscall;real-time detect rootkit,porcess injection,create file
...
asmlinkage int (*orig_execve)(const char __user *filename, const char __user *const __user *argv,
...
#if LINUX_VERSION_CODE == KERNEL_VERSION(3, 10, 0)
extern asmlinkage int monitor_finit_module_hook(int fd, const char __user *uargs, int flags);
#endif

extern asmlinkage int monitor_init_module_hook(void __user *umod, unsigned long len, const char __user *uargs);

#if (HOOK_CONNECT == 1)
static struct ftrace_hook CONNECT_HOOK = HOOK("sys_connect", monitor_connect_hook, &orig_connect);
#endif

#if (HOOK_EXECVE == 1)
static struct ftrace_hook EXECVE_HOOK = HOOK("sys_execve", monitor_execve_hook, &orig_execve);
#endif

#if (HOOK_ACCEPT == 1)
static struct ftrace_hook ACCEPT_HOOK = HOOK("sys_accept", monitor_accept_hook, &orig_accept);
#endif

#if (HOOK_ACCEPT == 1)
static struct ftrace_hook ACCEPT4_HOOK = HOOK("sys_accept4", monitor_accept4_hook, &orig_accept4);
#endif

#if (HOOK_INIT_MODULE == 1 && LINUX_VERSION_CODE == KERNEL_VERSION(3, 10, 0))
static struct ftrace_hook FINIT_MODULE_HOOK = HOOK("sys_finit_module",  monitor_finit_module_hook, &orig_finit_module);
#endif

#if (HOOK_INIT_MODULE == 1)
static struct ftrace_hook INIT_MODULE_HOOK = HOOK("sys_init_module", monitor_init_module_hook, &orig_init_module);
#endif

#if (HOOK_CREATE_FILE == 1)
static struct ftrace_hook OPEN_HOOK = HOOK("sys_open", monitor_open_hook, &orig_open);
#endif

#if (HOOK_CREATE_FILE == 1)
static struct ftrace_hook OPENAT_HOOK = HOOK("sys_openat", monitor_openat_hook, &orig_openat);
#endif

#if (HOOK_CREATE_FILE == 1)
static struct ftrace_hook CREAT_HOOK = HOOK("sys_creat", monitor_creat_hook, &orig_creat);
#endif

#if (HOOK_DNS == 1)
static struct ftrace_hook RECV_FROM_HOOK = HOOK("sys_recvfrom", monitor_recvfrom_hook, &orig_recvfrom);
#endif
...
static int lkm_init(void)
{
    ...
    if (err) {
        fh_remove_hook(&CONNECT_HOOK);
        return err;
    }
#endif

#if (HOOK_DNS ==  1)
    err = fh_install_hook(&RECV_FROM_HOOK);
    if (err) {
        fh_remove_hook(&RECV_FROM_HOOK);
        return err;
    }
#endif
#if (HOOK_ACCEPT == 1)
    err = fh_install_hook(&ACCEPT4_HOOK);
    if (err) {
        fh_remove_hook(&ACCEPT4_HOOK);
        return err;
    }

    err = fh_install_hook(&ACCEPT_HOOK);
    if (err) {
        fh_remove_hook(&ACCEPT_HOOK);
        return err;
    }
#endif
#if (HOOK_FINIT_MODULE == 1)
#if LINUX_VERSION_CODE == KERNEL_VERSION(3, 10, 0)
    err = fh_install_hook(&FINIT_MODULE_HOOK);
    if (err) {
        fh_remove_hook(&FINIT_MODULE_HOOK);
        return err;
    }
#endif
#endif
#if (HOOK_INIT_MODULE == 1)
    err = fh_install_hook(&INIT_MODULE_HOOK);
    if (err) {
        fh_remove_hook(&INIT_MODULE_HOOK);
        return err;
    }
#endif
#if (HOOK_EXECVE == 1)
    err = fh_install_hook(&EXECVE_HOOK);
    if (err) {
        fh_remove_hook(&EXECVE_HOOK);
        return err;
    }
#endif

#if (HOOK_CREATE_FILE == 1)
    err = fh_install_hook(&OPEN_HOOK);
    if (err) {
        fh_remove_hook(&OPEN_HOOK);
        return err;
    }
    err = fh_install_hook(&OPENAT_HOOK);
    if (err) {
        fh_remove_hook(&OPENAT_HOOK);
        return err;
    }
    err = fh_install_hook(&CREAT_HOOK);
    if (err) {
        fh_remove_hook(&CREAT_HOOK);
        return err;
    }
#endif
...
MODULE_DESCRIPTION("hook execve,connect,accept,accept4,ptrace,init_module,finit_module,open,openat,creat,recvfrom syscall;real-time detect rootkit,porcess injection,create file");
