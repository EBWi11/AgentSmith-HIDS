...
* 实现了两种将Hook Info从内核态传输到用户态的方式：netlink和共享内存，共享内存传输损耗相较于netlink减小30%，在测试服务器上Hook connect耗时中位数8478ns，更详细的AgentSmith-HIDS BencherMark请见:https://github.com/DianrongSecurity/AgentSmith-HIDS/tree/master/doc



### 关于兼容系统及内核版本

* AgentSmith-HIDS 仅在Centos7.2/7.3/7.4/7.5/7.6上进行过充分的测试，关于Kernel版本仅在3.10-327/3.10-514/3.10-693/3.10-862/3.10-957上进行过充分测试。虽然理论上Smith支持更多的版本，但是由于未经过充分测试，在加载LKM的时候会对Kernel进行版本强制校验(>3.10)，如果有其他人在其他版本上进行过稳定性测试，可以随时联系我们（需附稳定性测试报告）。
